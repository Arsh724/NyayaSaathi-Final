// PASTE THIS ENTIRE FILE INTO src/components/GenericEditModal.jsx
...
const GenericEditModal = ({ isOpen, onClose, onSuccess, itemData, columns, endpoint, title }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || `Failed to update ${title}.`, { id: loadingToast });
    } finally {
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              {editableColumns.map(col => {
                  ...
                  if (typeof value === 'boolean') {
                      return (
                          <div key={accessor} className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                              ...
                              <label htmlFor={accessor} className="block text-sm font-medium text-slate-700 dark:text-slate-300">{col.header}</label>
                          </div>
                      );
                  }
