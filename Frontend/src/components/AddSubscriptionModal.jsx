// PASTE THIS ENTIRE FILE INTO src/components/AddSubscriptionModal.jsx
...
const AddSubscriptionModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to create subscription.', { id: loadingToast });
    } finally {
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={handleClose}>
          <motion.div variants={modalVariants} initial="hidden" animate="visible" exit="exit" className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg border border-slate-200 dark:border-slate-700" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center p-4 border-b border-slate-200 dark:border-slate-700">
              <h3 className="text-xl font-bold text-slate-900 dark:text-white">Create New Subscription</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Organization/User ID</label>
                <input name="organizationRef" placeholder="Enter the MongoDB ID of the Kiosk/User" value={formData.organizationRef} onChange={handleChange} required className="input-style -mt-2" />
                ...
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Amount Paid</label>
                <input name="amountPaid" type="number" placeholder="e.g., 500" value={formData.amountPaid} onChange={handleChange} required className="input-style -mt-2" />

                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Expiry Date</label>
                <input name="expiryDate" type="date" value={formData.expiryDate} onChange={handleChange} required className="input-style -mt-2" />
